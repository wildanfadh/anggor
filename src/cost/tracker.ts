...
export class CostTracker {
	...
	track(provider: string, model: string, promptTokens: number, completionTokens: number): void {
		const costs = MODEL_COSTS[model] ?? { prompt: 0, completion: 0 };
		const cost = (promptTokens / 1000) * costs.prompt + (completionTokens / 1000) * costs.completion;
