...
// biome-ignore lint/complexity/noExcessiveCognitiveComplexity: manifest detection is inherently a big lookup table
...
async function parseGoMod(rootPath: string): Promise<ManifestResult | null> {
  try {
    ...
    const depLines = lines.filter((line) => line.startsWith("require ") || !line.startsWith("module ") && !line.startsWith("go ") && !line.startsWith("//") && !line.startsWith("exclude ") && !line.startsWith("replace ") && !line.startsWith("retract ") && !line.startsWith("toolchain "));
