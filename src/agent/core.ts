...
import { rollback as checkpointRollback, cleanupCheckpoint, createCheckpoint } from "../memory/checkpoint.js";
import type { Provider, ProviderMessage } from "../providers/index.js";
import * as fileTools from "../tools/file.js";
import { gitBranch, gitCommit, gitDiff, gitLog, gitStatus } from "../tools/git.js";
...
export class Agent {
	...
	private async llmExecute(task: string): Promise<AgentResult> {
		...
		while (iteration < maxIterations && !this.interrupted) {
			...
			try {
				...
				if (!action) {
					this.memory.addMessage("system", `Failed to parse LLM response: ${response.content.slice(0, 200)}`);
					continue;
				}

				this.memory.addMessage("assistant", `[${action.action}] ${action.thought ?? ""}`);
				...
				switch (action.action) {
					...
					case "tool_call": {
						...
						const result = await this.executeTool(action.tool as ToolName, action.input ?? {});
		...
		return this.makeResult(false, `Reached max iterations (${maxIterations}) without completing the task.`);
	}
	...
	private async heuristicExecute(task: string): Promise<AgentResult> {
		...
		if (!this.dryRun) {
			try {
				const checkpoint = await createCheckpoint(this.cwd, [...this.modifiedFiles], task);
	...
	async planOnly(task: string): Promise<AgentResult> {
		...
		if (this.provider) {
			...
			try {
				...
				if (action?.plan) {
					this.planner.createPlan(action.plan.task, action.plan.steps);
					this.memory.addMessage("system", `LLM-generated plan (dry-run):\n${this.planner.formatDryRunPlan()}`);
				} else {
	...
	private async reactLoop(task: string): Promise<AgentResult> {
		...
		while (iter < maxIterations && !this.interrupted) {
			...
			this.memory.addToolCall(action.tool, action.input, result.output, result.success, result.duration);
	...
	private async decideNextAction(task: string): Promise<{ tool: ToolName; input: string } | null> {
	...
	private mapStepToAction(step: PlanStep, _task: string): { tool: ToolName; input: string } {
	...
	private async executeAction(action: { tool: ToolName; input: string }): Promise<ToolCallResult> {
		const startTime = performance.now();
	...
	private async executeTool(tool: ToolName, input: Record<string, unknown>): Promise<ToolCallResult> {
		switch (tool) {
			case "file.read":
	...
	private async executeFileDelete(input: Record<string, unknown>): Promise<ToolCallResult> {
		try {
			...
			this.modifiedFiles.add(path);
			const msg = result.backupPath ? `Moved to backup: ${result.backupPath}` : "Moved to trash";
	...
	private async selfHeal(action: { tool: ToolName; input: string }): Promise<boolean> {
	...
	private parseLlmResponse(raw: string): LlmAction | null {
		...
		try {
			...
			if (parsed.action !== "plan" && parsed.action !== "tool_call" && parsed.action !== "done") {
				return null;
			}
	...
	private getModifiedPath(_tool: string, input: Record<string, unknown>): string | null {
