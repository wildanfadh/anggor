...
import { access, readFile as fsReadFile, writeFile as fsWriteFile, mkdir, rename, unlink } from "node:fs/promises";
...
export async function createFile(path: string, content: string): Promise<FileWriteResult> {
	const fullPath = resolvePath(path);
...
export async function deleteFile(path: string, safeMode = true): Promise<FileDeleteResult> {
	const fullPath = resolvePath(path);
...
export async function applyPatch(path: string, diff: string): Promise<FileWriteResult> {
	const fullPath = resolvePath(path);
