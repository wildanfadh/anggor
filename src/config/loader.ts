...
import { ApprovalModeSchema, ConfigSchema, ProviderNameSchema } from "./schema.js";
...
async function loadRawConfigSources(options: LoadConfigOptions): Promise<RawConfigSources> {
	...
	const [user, project] = await Promise.all([readJsonFile(userConfigPath), readJsonFile(projectConfigPath)]);
