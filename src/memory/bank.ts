...
import type { SessionMemory, SessionMessage, ToolCallRecord, PlanRecord, TodoItem } from "../agent/memory.js";
...
export async function clearMemory(): Promise<void> {
  ...
  try {
    ...
    await writeFile(path, JSON.stringify({ version: 1, updatedAt: Date.now(), cwd: process.cwd(), messages: [], toolCalls: [], plans: [], todos: [] }), "utf8");
  } catch {
