...
export async function main(): Promise<void> {
	...
	const [{ routeCommand }, { loadConfig }, { createProvider }] = await Promise.all([
	...
	if (command.flags.includes("cost")) {
		...
		const estimatedCost = tracker.estimateCost(model, estimatedPromptTokens, estimatedCompletionTokens);
